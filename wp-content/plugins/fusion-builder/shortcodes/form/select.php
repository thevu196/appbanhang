	...
	if ( ! class_exists( 'FusionForm_Select' ) ) {
		...
		class FusionForm_Select extends Fusion_Form_Component {
			...
			public function render_input_field( $content ) {
				...
				$element_html .= '<select tabindex="' . $this->args['tab_index'] . '" name="' . $this->args['name'] . '"' . $element_data['class'] . $element_data['required'] . $element_data['style'] . $element_data['holds_private_data'] . '>';
