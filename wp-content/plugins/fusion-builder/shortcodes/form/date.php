	...
	if ( ! class_exists( 'FusionForm_Date' ) ) {
		...
		class FusionForm_Date extends Fusion_Form_Component {
			...
			public function render_input_field( $content ) {
				...
				// Input markup.
				$element_html = '<input autocomplete="no" type="date" tabindex="' . $this->args['tab_index'] . '" name="' . $this->args['name'] . '" data-type="' . esc_attr( $this->args['picker'] ) . '" value="' . $this->args['value'] . '"' . $element_data['holds_private_data'] . $element_data['class'] . $element_data['required'] . $element_data['placeholder'] . $element_data['style'] . $auto_complete . '/>';
